  ...
  <div className="purchases-view">
    ...
    <div className="purchases-content" style={{ position: 'relative', zIndex: 5, width: '100%' }}>
        ...
        <div className="full-player-scroll-area">
          {beats.find(b => b.id === currentBeatId) && (
            ...
            <div className="full-tags-display">
              {beats.find(b => b.id === currentBeatId)?.tags?.split(',').filter(t => t.trim() !== "").map(tag => (
      ...
      <div className="edit-form-full">
        ...
        {/* Скрытые поля, чтобы не потерять текущие ссылки на файлы при сохранении */}
        {['mp3', 'wav', 'trackout', 'exclusive'].map((fileType) => (
          <input 
            key={fileType}
            type="hidden" 
            id={`edit-${fileType}`} 
            defaultValue={beats.find(b => b.id === currentBeatId)?.[fileType]} 
          />
        ))}
          ...
          <button className="apply-changes-btn" onClick={async () => {
            try {
              ...
              const updatedData = {
                ...
                // Ссылки берутся из скрытых полей, они останутся прежними
